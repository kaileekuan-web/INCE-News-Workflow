...
6. **No aggregator sources anywhere — news section and fundraising table alike.** The report links to the announcement itself — the company's own blog, `/news` or `/press` page, its press release, or the lead investor's own post — and falls back to trade press only when no primary source exists. TechCrunch's writeup of a launch the company blogged about is one hop further from the facts, often paywalled, and downstream of the thing we could have linked to. Deal databases (Crunchbase, PitchBook, Dealroom, Tracxn) are compiled from announcements and are **never** primary. 
   **This is a proof rule, not a denylist rule.** `AGGREGATOR_HOSTS` alone cannot promise "not an aggregator" — the 2026-08-14 live run proved it by shipping `pulse2.com` as a primary source purely because nobody had heard of it. So both sections use `is_own_announcement()`, which accepts a URL only on positive evidence:
   - the **subject's own domain** (`coderabbit.ai`, `discoveredmaterials.com`), or
   - a **named investor's own domain** (fundraising rows), or
   - a **press wire** carrying their release (Business Wire, PR Newswire, GlobeNewswire — the company pays to publish there and the text is theirs), or
   - **their own account on a publishing platform** — `huggingface.co/blog/CohereLabs/…` is Cohere announcing a model, and rejecting it because the domain says "huggingface" would delete exactly the announcements an AI report wants (`is_platform_owned()`).

   Everything else fails, known site or not. A syndicator's copy of a wire release (`morningstar.com/news/business-wire/…`) is a copy, not the release. Roundups are rejected by URL path on any host (`is_roundup_url()`), because a "venture funding roundup August 12" page is a list of other people's news.

   **Items that fail get one chance, then they are dropped:**
   - News section — `upgrade_to_primary_sources()` (`resolve_sources.py`) asks a different question than the ordinary resolver: *who is this story about, and did they announce it themselves?* "Cursor open-sources MoK" surfaces MarkTechPost; "Cursor's own post about MoK" surfaces Cursor. What still has no first-party link is dropped and tallied as `aggregator source (N)`. `--allow-aggregator-source` keeps them.
   - Fundraising table — `_resolve_primary_sources()` (`generate_word_doc.py`) runs the same search per row. Rows where the company genuinely never announced publicly keep their aggregator link and are reported on the console, because a real round with a weak source still belongs in the table.

   **Expect this to shrink the news section, sometimes to nothing.** On the 2026-08-13 dataset only 2 of 5 surviving items had first-party links. An empty section means the collection is mostly funding posts and third-party writeups — fix that upstream in `x_accounts.txt`, not by relaxing the rule.
...
**2026-08-14, later the same day — a denylist cannot promise "no aggregators".** The first live verification run recovered only 5 of the 12 missed companies (Discovered Materials, River AI, CodeRabbit, Skan AI, Mindgard) and, worse, sourced 15 of its 17 rows to aggregators despite the new ranking. Three separate faults:
- **Ranking can only choose among what the search returned**, and everything returned was downstream. Fixed by going to look: `_resolve_primary_sources()` searches for the announcement behind every row that lacks one.
- **The newsletter pass read a deal list and returned one company from it.** The Gravity row is sourced to an Axios Pro Rata issue whose URL literally contains `point2` — Point2 was on the page and was dropped. The prompt now says to enumerate every company in a deal list.
- **`pulse2.com` passed as "primary" because it was not on the denylist.** This is the fault that mattered: absence of evidence is not evidence. Replaced with `is_own_announcement()`, which demands the subject's domain, an investor's domain, a press wire, or their own platform account. Under it, 16 of those 17 rows correctly fail.

Also learned: **the funding search's bottleneck was concurrency, not passes.** `NEWS_MAX_WORKERS` defaults to 6, which turned 25 turns into 5 sequential waves and 35+ minutes. At `NEWS_MAX_WORKERS=25` the same 25 turns finish in under 4 minutes. Set it for any multi-pass funding run.

**Still open:** the 12 missed companies have not been re-verified since these fixes — that needs a paid funding search over 8/10–8/14. The `优先级` (1-5 relevance) column in `generate_word_doc.py`'s *news* table is untouched; it is a different column from the fundraising one that was removed, but it shares the label.
